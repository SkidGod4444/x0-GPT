...
import { ArchiveIcon, BookCopy, BrainCircuit, FileText, InboxIcon, SquarePen, SquareTerminal } from "lucide-react";
import {
...
export default function ChatComp() {
  ...
  return (
    <div className="flex min-h-screen w-full">
      {/* Left Side Content */}
      <aside
        className={`fixed inset-y-0 left-0 z-10 flex flex-col border-r bg-[#171717] transition-all duration-300 overflow-hidden ${
      ...
      >
        ...
        <div className="flex h-[60px] w-full items-center justify-between px-4 ">
          {/* Left Side */}
          <div
            className={`flex items-center bg-[#171717] rounded-xl transition-all duration-700 overflow-hidden ${
        ...
        <div className="flex h-full w-full items-center justify-center">
          <div className={`grid grid-cols-2 p-10 md:grid-cols-4 gap-4 mb-40 ${
              isCollapsed ? "mr-0" : "mr-20"
            }`}>
            {ITEMS.map((item, index) => (
              <div key={index}>
                <div className="flex h-full w-40 hover:bg-muted border-2 border-secondary rounded-3xl select-none flex-col items-center justify-center p-3">
        ...
        <div className="flex h-full w-full items-center justify-between">
          ...
          <div
            className={`flex flex-col mb-10 items-center justify-center transition-all duration-300 ${
              isCollapsed ? "w-full mx-40" : "w-full ml-80 mr-40"
            }`}
          >
            <Label className="flex h-full w-full bg-secondary rounded-full p-2 items-end justify-between">
              ...
              <Textarea
                className="h-5 w-full mx-2 focus-visible:outline-none"
          ...
          {/* Right Side */}
          <div
            className={`flex items-center justify-end transition-all duration-700 overflow-hidden ${
              isCollapsed ? "opacity-100" : "opacity-0"
            }`}
          >
            {isCollapsed ? (
